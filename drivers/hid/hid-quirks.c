...
#if defined(USB_VENDOR_ID_COOLER_MASTER) && \
	defined(USB_DEVICE_ID_COOLER_MASTER_MICE_DONGLE)
...
#if defined(USB_DEVICE_ID_CHICONY_HP_5MP_CAMERA) && \
	defined(USB_DEVICE_ID_CHICONY_HP_5MP_CAMERA2)
		...
		if ((hdev->product == 0x0401 || hdev->product == 0x0400))
			for (i = 0; strlen(elan_acpi_id[i].id); ++i)
				if (!strncmp(hdev->name, elan_acpi_id[i].id,
					...
					return true;
		break;
#if defined(USB_VENDOR_ID_JIELI_SDK_DEFAULT) && \
	defined(USB_DEVICE_ID_JIELI_SDK_4155)
